...
<?php    
        foreach ($animauxMarais as $key=>$animalMarais) {
            ?>
...
<div class=" card mycard mb-4 mt-4" style="width: 20rem;">
            <img src="<?= $animalMarais["chemin"]?>" class="imgcontents img-fluid " alt="...">
            <h3><?= $animalMarais["prénom"]?></h3>
         <div class="card-body">
            <p class="card-text">Race :<br> <?= $animalMarais["race"]?></p>
        </div>
        <div><p>l'avis du veterinaire :<br> <?= $animalMarais["commentaire"]?></p></div>
    </div>
</div> 
<?php 
 };
